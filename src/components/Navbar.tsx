  ...
  return (
    ...
    >
      <div className="mx-auto max-w-screen-xl px-4 sm:px-6">
        <div
          className={`mt-3 sm:mt-4 rounded-2xl px-5 sm:px-7 h-[4.5rem] sm:h-[4.75rem] flex items-center justify-between transition-all duration-500 ${
            scrolled
              ? "glass-strong shadow-2xl shadow-background/80"
              : "glass-strong shadow-xl shadow-background/60"
          }`}
          style={{
            borderBottom: scrolled ? "1px solid hsl(var(--border) / 0.3)" : undefined,
          }}
        >
          <Link to="/" className="flex items-center group shrink-0">
            <img
              ...
              style={{ filter: "drop-shadow(0 0 16px hsl(250 80% 65% / 0.3))" }}
            />
          </Link>
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="text-micro font-medium text-muted-foreground hover:text-foreground px-3 py-1.5 rounded-lg transition-all duration-200 hover:bg-secondary/50 border border-border/30"
            >
              {lang === "en" ? "বাংলা" : "EN"}
            </button>
            <Link
            ...
            <Link
              to="/signup"
              className="hidden sm:inline-flex cta-shine text-caption font-semibold bg-gradient-to-r from-primary to-accent text-primary-foreground px-5 py-2.5 rounded-xl hover:brightness-110 hover:scale-[1.03] transition-all duration-200 shadow-md shadow-primary/25 hover:shadow-lg hover:shadow-primary/40"
            >
              {t.startFree}
            </Link>
            <button
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              ...
              className="md:hidden mt-2 rounded-2xl glass-strong shadow-2xl shadow-background/60 p-4 flex flex-col gap-1"
            >
              {navLinks.map((item) => (
                <a
                  key={item.href}
